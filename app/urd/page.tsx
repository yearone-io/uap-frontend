...
const UAPConfigPage = () => {
  ...
  const turnOffAssistant = async (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    if (!isConnected) {
      toast({
        title: 'Wallet not connected',
    ...
    try {
      ...
      await toggleUniveralAssistantsSubscribe(
        provider,
        upAddress,
        supportedNetworks[network.chainId].protocolAddress,
        supportedNetworks[network.chainId].defaultURDUP,
        true
      );
  ...
  const handleSubmitConfig = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const executiveConfigAddress = "0xfE67D89DeBEC38592aB2FeD217b8bbb28851DF88".toLowerCase();
      const executiveConfigSchema = {
        name: 'UAPExecutiveConfig:<address>',
        // '0x' + ethers.keccak256(ethers.toUtf8Bytes('UAPExecutiveConfig')).substring(2, 34),
        key: generateMappingKeyFromAddress('UAPExecutiveConfig', executiveConfigAddress),
        keyType: 'Mapping',
        valueType: 'address',
        valueContent: 'Address',
      }
      ...
      const encodedData = erc725.encodeData([{
        ...
        keyName: executiveConfigSchema.name,
        dynamicKeyParts: [assistantAddresses[0]],
        value: executiveConfigAddress,
      }]);
      ...
      const tx = await UP.connect(signer).setDataBatch(encodedData.keys, [encodedValues, encodedData.values[1]]);
  ...
  const breadCrumbs = (
    <>
      ...
      >
        ...
        <BreadcrumbItem>
          <BreadcrumbLink href={`/profile/${network.chainId}/${address}`}>
            {'Profile'}
          </BreadcrumbLink>
        </BreadcrumbItem>
  ...
  return (
    <>
      ...
      >
        ...
        <form onSubmit={turnOffAssistant}>
            <Text fontSize="lg" fontWeight="bold" mb={4}>
             Disable Assistant
            </Text>
            <Button colorScheme="teal" type="submit">
              Disable Universal Assistants
            </Button>
        </form>
      </Flex>
    </>
